...
print
"log,Model,Examination,Techniques,Test started,Test fail,Test fin,duration(ms),Initial,cpn,skel,scc,sdd,version\n";
...
foreach my $file (@files) {
	if ( $file =~ /out$/ ) {
		...
		my $tot    = 0, my $fail = 0, my $fin = 0, my $dur = 0, my $init = 0,
		  my $taut = 0, my $sdd  = 0, my $bmc = 0, my $kind = 0, my $pins = 0,
		  my $por  = 0, my $itstime = 0, my $itsmem = 0;
		my $skel = 0, my $scc = 0, my $cpn = 0;
		open IN, "< $file";
		while ( my $line = <IN> ) {
			chomp $line;
			if ( $line =~ /syscalling/ ) {
		...
		close IN;
		print
"$file,$model,$exam,$tech,$tot,$fail,$fin,$dur,$init,$cpn,$skel,$scc,$sdd,$version\n";
	}
}
